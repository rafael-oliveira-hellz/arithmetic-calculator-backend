# Calculator logout - Serverless Microservice

## **Overview**

The **Calculator Logout** microservice is a serverless application designed exclusively for **user logging out**. It operates on AWS Lambda, ensuring scalability, cost-efficiency, and seamless integration with AWS services. The microservice includes core functionaly such as log out users from Cognito.
...
- **User Logout**: Implements APIs for user logout.
...
```bash
docker build -t calculator-logout .
```

**This command**:

- Uses the Dockerfile to build an image named calculator-logout.
...
```bash
docker run -p 9000:8080 calculator-logout
```
...
Follow these steps to test the Lambda function locally for user logout using Docker.
...
Create a JSON file with the test payload containing the user logout data, in the project root, for instance.
...
For example, a successful logout might return:
...
```bash
docker tag calculator-logout:latest <account_id>.dkr.ecr.<region>.amazonaws.com/calculator-logout:latest
```

### **Step 6.3: Push the Image to ECR**

```bash
docker push <account_id>.dkr.ecr.<region>.amazonaws.com/calculator-logout:latest
```
